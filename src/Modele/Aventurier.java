...
public class Aventurier {
    ...
    // Etat= COULEE pour déplacement (helico) et etat =  INONDEE pour l'assèchement (sac de sable)
    public ArrayList<Tuile> posPossibleAll(EtatTuile etat) {
        ...
        for (int x = 0; x < 6; x++) {
            for (int y = 0; y < 6; y++) {
                if (etat == INONDEE) {
                    if (controleur.getGrille().getTuileCase(x, y) != null && controleur.getGrille().getTuileCase(x, y).getEtat() == etat) {
                        p.add(controleur.getGrille().getTuileCase(x, y));
                    }
                } else if (etat == COULEE) {
                    if (controleur.getGrille().getTuileCase(x, y) != null && controleur.getGrille().getTuileCase(x, y).getEtat() != etat) {
                        p.add(controleur.getGrille().getTuileCase(x, y));
